  ...
  const getMessageHtml = (mes: Message, saying: string, abb: string, color:string) => {
    ...
    return `
      <form action="/sendmes/" method="post" style="display:inline">
        ...
        <input type="submit" value="${abb}" title="${saying}" style="width:10.5%;height:40px;background-color: #${color};border-radius:5px;color:#575169;padding:1%">
      </form>
    `
  }
  ...
  const SendHtml = (mes: Message, saying: string, abb: string, color:string) => {
    ...
    return `
      <form action="/sendmes/" method="post">
        <input id="${mes.id}" type="text" name="text" value="${saying}" style="width:86%;height:40px;background-color: #f9f5ea;border-radius:5px;color:#ffffff;padding:1%;border:0">
        <input id="${mes.id}" type="hidden" name="toId" value="${(from && from.id) || ''}">
        <input type="submit" value="${abb}" title="${saying}" style="width:10%;height:40px;background-color: #${color};border-radius:5px;color:#575169;padding:1%;border:0">
      </form>
    `
  }
  ...
  const ChangeHtml = (mes: Message, saying: string, abb: string, color:string, alias:string) => {
    ...
    return `
      <form action="/mesalias/" method="post" style="display:inline">
        ...
        <input type="submit" value="${abb}" title="${saying}" style="width:10.5%;height:40px;background-color: #${color};border-radius:5px;color:#575169;padding:1%;border:0">
      </form>
    `
  }
